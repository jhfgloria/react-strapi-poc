...
module.exports = {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Recipe.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Recipe._attributes)
      .filter(attribute => attribute !== Recipe.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Recipe._attributes[attribute].type));
